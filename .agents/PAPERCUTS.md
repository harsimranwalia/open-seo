...
- [ ] `2026-08-11T05:12:03Z` — `claude` — The repo has no `.gitattributes`, so a Windows checkout (Git for Windows defaults `core.autocrlf=true`) rewrites `docker-entrypoint.sh` to CRLF and the self-host container crash-loops on `set -e\r` with the cryptic `docker-entrypoint.sh: 8: set: Illegal option -`, which never names line endings. Pin shell scripts to LF with a `.gitattributes` rule (`*.sh text eol=lf`) so the documented `docker compose up -d` quickstart works on Windows.
