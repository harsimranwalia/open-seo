...
function SignUpPage() {
  ...
  const { redirectTo } = useAuthPageState(search.redirect);
  const postSignupRedirect = redirectTo === "/" ? "/onboarding" : redirectTo;

  // Turnstile is active only when a site key is configured.
    ...
    onSubmit: async ({ formApi, value }) => {
      ...
      try {
        ...
        const result = await authClient.signUp.email({
          ...
          callbackURL: postSignupRedirect,
